...
CLASS zcl_054906_carrier IMPLEMENTATION.
  ...
  METHOD get_biggest_cargo_plane.
    DATA max_weight_in_tons TYPE i.

    LOOP AT airplanes INTO FINAL(airplane).
      IF airplane IS INSTANCE OF zcl_054906_cargo_plane.
        DATA(cargo_plane) = CAST zcl_054906_cargo_plane( airplane ).
        IF cargo_plane->get_total_weight_in_tons( ) > max_weight_in_tons.
          biggest_cargo_plane = cargo_plane.
          max_weight_in_tons = cargo_plane->get_total_weight_in_tons( ).
        ENDIF.
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
